...
class TestScatterPlot(unittest.TestCase):
	...
	def test_valid_no_normalise(self):
		s = ScatterPlot([os.path.join(data_dir, 'condition1.plot'), os.path.join(data_dir, 'condition2.plot')], [os.path.join(data_dir, 'control1.plot'), os.path.join(data_dir, 'control2.plot')], 10,  'scattertest', False, verbose = True)
		self.assertTrue(s.create_scatter_plot())
		self.assertTrue(s.create_linear_plot())
		self.assertTrue(s.create_abs_scatter_plot())
