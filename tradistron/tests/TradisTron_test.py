...
class TestOptions:
	def __init__(self, plotfiles, minimum_threshold, window_size,window_interval, verbose, prefix, minimum_logcpm, minimum_logfc, pvalue, iterations, normalise_plots,minimum_block,span_gaps ):
...
class TestTradisTron(unittest.TestCase):
	...
	def test_small_real(self):
		...
		t = TradisTron(TestOptions([case, control], 3, 100, 100, False, 'testoutput', 1, 1, 1, 1, True,1,0))
	...
	def test_small_2iterations(self):
		case = os.path.join(data_dir, 'small_case.insert_site_plot.gz')
		control = os.path.join(data_dir, 'small_control.insert_site_plot.gz')
		t = TradisTron(TestOptions([case, control], 3, 100, 100, False, 'testoutput', 1, 1, 1, 2, False,1,0))
		self.assertTrue(t.run())
		self.assertTrue(os.path.exists('testoutput_1'))
		shutil.rmtree("testoutput_1")
